...
output is bounded by the quality of these calls. Defaults are what worked for run 1.0.
...
| Tier definitions | Tier 1 = dual-lens top (views ∪ ratio); Tier 2 = next by ratio; Tier 3 = next by views | 80 ideas/tier worked for run 1.0. Scale to your idea appetite. |
...
5. **The niche's exam-cycle calendar** — registration opens, exam date, answer-key release, result
   ...
   free to consume content). For run 1.0 this was the school-board academic calendar for the target
   grade level.
...
- Dedupe primary keywords first (run 1.0: 240 ideas → 37 unique keywords, saving ~50% of credits).
